...
### Requirement: An admin can mark an account as accepting sandbox purchases

The plan console SHALL let an authorized admin set and clear a **sandbox account** mark
...
- **WHEN** an authorized admin sets the sandbox-account mark on an account
- **THEN** the account reads as a sandbox account, and the audit trail carries the change with the acting admin

#### Scenario: Clearing the mark is audited too

- **WHEN** an authorized admin clears the mark
- **THEN** the account no longer reads as a sandbox account, and the audit trail carries that change
...
- **WHEN** an account is marked as accepting sandbox purchases and has no entitlement row
