...
### Requirement: The directory can be filtered to store testers

The account directory SHALL accept a filter that restricts the listing to accounts
marked as store testers, so the mark can be reviewed without knowing in advance which
...
- **WHEN** an admin lists accounts with the store-tester filter set
- **THEN** every returned account is marked a store tester, and the total counts only those

#### Scenario: The filter composes with the text query

- **WHEN** an admin combines the store-tester filter with a handle query
