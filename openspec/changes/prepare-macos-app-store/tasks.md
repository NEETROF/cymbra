...
- [ ] 1.1 Add `LSApplicationCategoryType = public.app-category.education` to `apps/music/macos/Runner/Info.plist` (D7)
- [ ] 1.2 Add `ITSAppUsesNonExemptEncryption = false` to `apps/music/macos/Runner/Info.plist`, mirroring `ios/Runner/Info.plist` (D7)
- [ ] 1.3 `plutil -lint apps/music/macos/Runner/Info.plist` passes

## 2. Entitlements audit (no Apple account needed)

- [ ] 2.1 Add `keychain-access-groups = [$(AppIdentifierPrefix)com.cymbra.music]` to `apps/music/macos/Runner/Release.entitlements` (D5)
- [ ] 2.2 Add the same key to `apps/music/macos/Runner/DebugProfile.entitlements` so debug and release stay consistent
- [ ] 2.3 Diff the two entitlement files and confirm the only remaining differences are debug-only capabilities (`allow-jit`, `network.server`) that the shipped app does not need — in particular confirm `network.server` stays out of Release (D5)
- [ ] 2.4 `plutil -lint` both entitlement files

## 3. Local signed archive (needs the Apple account — section 6 first)

- [ ] 3.1 Write `apps/music/macos/ExportOptions.plist` for local archives: `method: app-store`, `teamID VMFJ6KRW77`, automatic signing, `installerSigningCertificate` set to the Mac Installer Distribution identity (mirrors `ios/ExportOptions.plist`)
...
- [ ] 4.1 Replace the "Disable code signing" step in the `macos` job of `.github/workflows/release-build.yml` with a secrets presence check that fails with an explicit error naming the missing material (mirrors the `ios` job's "Check iOS signing secrets")
- [ ] 4.2 Add a step importing the Apple Distribution cert, the Mac Installer Distribution cert and the Mac App Store provisioning profile into a throwaway keychain, reusing the `ios` job's `security create-keychain` / `set-key-partition-list` sequence (D3)
- [ ] 4.3 Append `CODE_SIGN_STYLE = Manual`, `DEVELOPMENT_TEAM`, `PROVISIONING_PROFILE_SPECIFIER`, `CODE_SIGN_IDENTITY = Apple Distribution` to `macos/Runner/Configs/Release.xcconfig` at build time (D2)
- [ ] 4.4 Generate `macos/ExportOptions-ci.plist` with `signingStyle: manual` and the bundle-id → profile-name mapping (D3)
- [ ] 4.5 Replace the `flutter build macos` + `ditto` steps with the build → archive → export sequence from 3.2, producing `cymbra-${TAG}-macos.pkg`
- [ ] 4.6 Drop the `.zip` release asset and do **not** attach the `.pkg` (D4)
- [ ] 4.7 Add the delivery step: `xcrun altool --upload-app --type macos` gated on `github.event_name == 'push'`, skipping with a warning when the `ASC_API_*` secrets are absent
- [ ] 4.8 Update the workflow header comment (lines 42-43) which currently states the macOS artifact is unsigned
- [ ] 4.9 `actionlint` (or a YAML parse) clean on the edited workflow

## 5. Documentation

- [ ] 5.1 Add a "macOS signing (Mac App Store)" section to `apps/music/README.md` beside the iOS one: Apple-side prerequisites, the six new secrets, and the local command from 3.2
- [ ] 5.2 Document in the same section that the release publishes no macOS download until the listing is live, and why (D4)
...
- [ ] 9.1 `openspec validate prepare-macos-app-store --strict` passes
- [ ] 9.2 `melos run analyze` and `dart format` clean (no Dart change expected — confirm the tree is untouched)
