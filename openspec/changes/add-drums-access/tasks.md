...
- [x] 9.9 Record the general-availability sequence where the operator will read it (runbook or the flag's doc line): **widen the scope to `global` FIRST, close the campaign SECOND**. `beta:<key>` matches staff *or* member, so closing the campaign while the scope is still `beta:` drops every tester at once and leaves only staff seeing the feature — a broken rollout produced by an action that looks correct — FAIT 2026-08-25 : consigné dans `backend/deploy/DEPLOY.md`, section « Taking a beta feature to general availability — the order matters », à côté de la procédure d'activation du module plans — c'est-à-dire là où l'opérateur travaille, pas seulement dans le commentaire de `DRUMS_ENABLED`. La section énonce l'ordre, la raison (`beta:<clé>` matche staff *ou* membre, donc fermer d'abord rend la fonctionnalité staff-only en silence), le maintien du flag comme kill-switch, et le rattrapage par `ReopenCampaign` si l'ordre a été inversé
