...
- [x] 1.1 Add rate-limit config knobs to `backend/platform/src/config.rs` beside the existing throttle config: download burst (max + window); play-aware volume allowance (`base_floor`, play multiplier `k`, `hard_ceiling`, allowance window); enumeration (max + window); and an enable/kill-switch boolean. *(Added `CatalogLimitsConfig` + `catalog_limits()` parser.)*
...
- [x] 3.1 Read `plays_in_window` per user from `PlayRepo::session_points` (signal = **in-window play-session count**; distinct-scores would need score_id, absent from `SessionPoint`).
- [x] 3.2 Cache `plays_in_window` per user in the shared cache (`catplays:{user}`) with a short TTL (60s) to avoid a `PlayRepo` round-trip on every download; on cache/`PlayRepo` unavailability, fall back to 0 plays ⇒ the `base_floor` allowance (fail-safe).
