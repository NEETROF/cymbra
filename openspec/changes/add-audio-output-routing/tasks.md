...
- [ ] 1.1 Add a `NoteSource` enum (`midiDevice` | `onScreen` | `computerKeyboard`)
  and thread it through `Player.noteOn`/`noteOff` with a default that keeps
  existing call sites compiling
- [ ] 1.2 Tag the six call sites: MIDI stream
...
- [ ] 1.3 Add the `instrumentSoundsItself` preference (Freezed state + persisted
  through the existing preferences store)
- [ ] 1.4 Gate only the synth call:
...
- [ ] 1.5 Verify score playback (`_audio.noteOn` at `:162`), metronome clicks and
  preview clips are untouched by the gate
- [ ] 1.6 Disable the setting with a stated reason while no MIDI port is connected

## 2. Engine: selectable output device

- [ ] 2.1 Move the device choice out of `run_audio_thread`
...
- [ ] 2.2 Put the pure resolution/fallback logic in `audio_core.rs` (requested
  name → available names → chosen name) so it is host-testable
- [ ] 2.3 Add an `AudioCommand` variant to rebuild the stream on a new device:
  all-notes-off first, rebuild `Stream` + `Synthesizer` on the new sample rate,
  keep the SoundFont in memory (no reload)
- [ ] 2.4 Add the public API — `list_audio_outputs()`,
...
- [ ] 2.5 Run `flutter_rust_bridge_codegen generate` (public API changed)
- [ ] 2.6 Rust tests on `audio_core.rs`: absent remembered device → default;
  unknown name → default; empty device list; default-follows-`None`
- [ ] 2.7 Keep `cargo llvm-cov --workspace --fail-under-lines 80` green

## 3. Platform route reporting (mobile)

- [ ] 3.1 Define the `{name, kind}` route shape with `kind` ∈ `builtin |
  headphones | bluetooth | usb | other`, unknown port types mapping to `other`
- [ ] 3.2 iOS: read `AVAudioSession.currentRoute` for name + port type, present
  `AVRoutePickerView`, and push route-change notifications to Dart
- [ ] 3.3 macOS/desktop: report the active cpal device as the route (kind derived
  from the device, `other` when unknown) so the UI has one shape everywhere
- [ ] 3.4 Android: read the active output device from `AudioManager`, present the
  system output switcher, and observe route changes
- [ ] 3.5 Confirm the AVAudioSession category still fits (playback, and that a
  route change does not stop the stream)

## 4. Dart seam and state

- [ ] 4.1 Add `lib/services/audio_routing_service.dart`: abstract seam over
...
- [ ] 4.2 Expose it as a `@riverpod` provider; add a Freezed routing state
  (available outputs, active route, selection failure)
- [ ] 4.3 Persist the selected output **by name** and restore it at startup,
  displaying the *active* device rather than the requested one
- [ ] 4.4 Persist `outputOffsetMs` (default 0) alongside `instrumentSoundsItself`

## 5. UI

- [ ] 5.1 Add a "Sound output" section to the player settings drawer and
...
- [ ] 5.2 Add the instrument-sounds-itself toggle to that section, with copy
  explaining it is for instruments that produce their own sound
- [ ] 5.3 Show the wireless warning when the active route's kind is `bluetooth` —
  driven by `kind`, never by matching the route name
- [ ] 5.4 Reveal the offset control only when a wireless route is active; suggest
  a starting value without applying it
- [ ] 5.5 Isolate side effects (snackbar on a failed device open, re-reading the
...
- [ ] 5.6 Add the fr/en l10n strings — no raw platform or engine error strings

## 6. Offset applied to the reference (land last)

- [ ] 6.1 Apply `outputOffsetMs` to the time reference the notifier passes to the
...
- [ ] 6.2 Apply the same value to the rendered playhead so highlight and judgment
  derive from one number
- [ ] 6.3 Confirm Wait Mode inherits the shift through the playhead, with no
  separate handling
- [ ] 6.4 Test: offset 0 reproduces today's verdicts exactly (guard against a
  silent scoring regression)
- [ ] 6.5 Test: with a non-zero offset, an attack aligned to the delayed audio is
  judged on time rather than late

## 7. Tests

- [ ] 7.1 Unit tests for the source gate: MIDI note not synthesized when the
  setting is on; on-screen and computer-keyboard notes still synthesized; scoring
  and Wait Mode identical either way
- [ ] 7.2 Unit test that score playback, metronome and preview clips are
  unaffected by the setting
- [ ] 7.3 Unit tests for the routing notifier with a mockito mock: device list,
  selection, failed open, absent remembered device, route change
- [ ] 7.4 Widget tests: desktop shows a device list, mobile shows the active route
  + picker button (override the platform seam); wireless warning shown for a
  `bluetooth` kind and not for `usb`/`headphones`
- [ ] 7.5 Widget test that the toggle is disabled with its reason when no MIDI
  port is connected
- [ ] 7.6 Keep Flutter line coverage ≥ 80%

## 8. Gates and manual verification

- [ ] 8.1 `melos run analyze`, `dart format`, `dart run custom_lint` clean
- [ ] 8.2 `cargo fmt --all --check` + `cargo clippy --workspace --all-targets --
  -D warnings`
...
- [ ] 8.8 Restore the iOS/macOS project files if a macOS test run rewrote them
