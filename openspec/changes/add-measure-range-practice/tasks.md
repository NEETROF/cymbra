...
- [ ] 1.1 Add practice-range fields to `PlayerData` (Freezed): `practiceStartMeasure`,
...
- [ ] 1.2 Make effective bounds range-aware: derive `startMs`/`endMs` from the range via
...
- [ ] 1.3 Add `Player.setPracticeRange(int start, int end)` and `clearPracticeRange()` that
  normalize/clamp to `[0, lastMeasure]` (`start ≤ end`), reset `elapsedMs` to the new range
  start, and call `_silenceAll()`.
- [ ] 1.4 Gate the scorer: in `_maybeStartRun`/`_atStart`, do **not** arm a scored run when
  `isSelectiveRun`; a selective run follows the existing unscored (`cancelRun`) path.
- [ ] 1.5 Range-aware loop-wrap: in `advance`, target the **range start** (not piece start) on
  wrap for a selective run; a selective run with looping off stops at the range end.
- [ ] 1.6 Add from/to measure **steppers** + a full-vs-selective toggle to the pre-play setup
  modal; wire to `setPracticeRange`/`clearPracticeRange`. Apply on dismiss.
- [ ] 1.7 Add a compact "bars a–b · loop" readout badge to the transport bar during a selective
  run (see design Open Question — transport badge).
- [ ] 1.8 Unit tests (host): range normalization/clamp, effective start/end mapping, whole-piece
  == today (no regression), selective run does not arm scoring, wrap targets range start.
- [ ] 1.9 Widget tests: setup-modal full/selective toggle + steppers set the range; transport
  badge reflects the range. Run `dart run custom_lint`; keep coverage ≥ 80%.

## 2. Phase 2 — Practice as scoreless activity (backend + proto)

- [ ] 2.1 Proto/gRPC: add a `recordPractice`-style RPC (client session id, score id, timestamp
...
- [ ] 2.2 Backend: implement `recordPractice` ingest — idempotent by client session id, stored
  separately from scored sessions; aggregate per-day practice counts. Rust tests
  (mockall for repo/port seams); coverage ≥ 80%.
- [ ] 2.3 App capture: at practice-session end, enqueue a **scoreless** activity record into the
...
- [ ] 2.4 Heatmap: surface the per-day practice count distinct from scored plays; a
  practice-only day renders as a neutral active cell (never a failure color); count in
  intensity/tooltip.
- [ ] 2.5 Tests: app — practice capture enqueues scoreless record, once-per-session, survives
...
- [ ] 3.1 Loop count: add `loopCount` (finite `N` / infinite) to state + `advance` (decrement
  per wrap, stop + silence at zero). Add loop-count control (presets 2/4/8/∞) to the setup sheet.
- [ ] 3.2 Practice tempo ramp: add `tempoRampStep` to state; increment `speed` at each wrap,
  clamped to `[startSpeed, maxSpeed]`, selective-only. Add the ramp control to the setup sheet.
- [ ] 3.3 Tap-to-select on the Partition view: expose per-measure hit rects from
...
- [ ] 3.4 Per-score persistence: save/load the range + loop settings keyed by score id via the
  `local-preferences`/store seam; clamp on load to the current measure count (fallback whole
  piece); pre-fill the setup sheet.
- [ ] 3.5 Summary entry: add a "practice this section" action to the session-summary modal that
  opens the range picker and starts a selective run; keep actions reachable on short viewports.
- [ ] 3.6 Tests (host + widget): loop-count stop-after-N, ramp clamp, ramp not applied to full
...
- [ ] 4.1 `openspec validate add-measure-range-practice --strict` passes.
- [ ] 4.2 `melos run analyze` + `dart format` clean; `cargo fmt --all --check` +
  `cargo clippy --workspace --all-targets -- -D warnings` clean.
- [ ] 4.3 Full test + coverage gate green (Rust `cargo llvm-cov` ≥ 80%; Flutter
