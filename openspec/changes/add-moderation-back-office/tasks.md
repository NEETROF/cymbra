...
> DEFERRED — this whole section (the `bo.cymbra.app` Vue SPA, wasm render, deploy)
> is a separate slice from the backend and is not verifiable by this repo's
> Rust/Flutter gates. The backend it depends on (roles, guards, `SetModerationStatus`,
> `GrantRole`/`RevokeRole`, `role_grants`, structured `sort`, gRPC-web + CORS) is
> delivered by sections 1–3 here.
...
- [ ] 5.3 Vue app: unit/component tests for the table filters, status filter gating, queue ordering, and the accept/reject flow (its own test setup, outside the Flutter/Rust gates). — DEFERRED (Vue slice)
