...
- **Two surfaces, one helper.** The signed-in account menu gets two `PopupMenuItem`s (below a `PopupMenuDivider`, near sign out / delete) so the links sit next to the user profile; entry gets a `Text.rich` consent line with two tappable spans reachable to guests. Both call the same resolver + launcher. Placing the durable link in the account menu (not the player's settings drawer) keeps it with the account rather than buried in per-score playback settings.
