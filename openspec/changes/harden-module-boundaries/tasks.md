...
- [ ] 3.10 Remove the grant lock from task 1.1 and its test 1.2; keep 1.3 adapted. **Ship this AFTER 3.1-3.9 are running in production, in its own PR**: the lock is the stopgap that stops a privileged non-music role from being created at all, so deleting it in the same change that builds its replacement would leave no moment where both hold. A guard missed in the migration would silently rearm the hole.
