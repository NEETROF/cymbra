...
### D2 — Review: FSRS via the `fsrs` crate, the same scheduling everywhere
...
computable at any moment by the core. The crate version is pinned; FSRS states store their
parameters (the crate evolves, and so do its defaults). The review's UI surfaces (side
panel, drawer, icon popup) belong to `add-lingua-extension-review` — the icon badge stays
dedicated to the page percentage, the due count lives in the popup and the side panel (no
"due" badge and no alarm in v1).
