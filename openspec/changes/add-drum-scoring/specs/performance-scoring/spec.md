...
Performance scoring SHALL be active in **every render mode the loaded score
offers** — for a keyboard score Synthesia, the horizontal scrolling staff, and
the engraved vertical Partition; for a percussion score the cascade and the
percussion notation modes (`add-drum-notation-render`) — with Wait Mode
**either on or off**.
...
which hand(s) were played, so the result reflects a single hand selection. For a
percussion score the selection reads **hands / feet / both** (`hand-selection`),
and the recorded selection carries that reading. Changing the
...
#### Scenario: A percussion run in the cascade is scored

- **WHEN** a percussion score is loaded and the player starts a full run from
  the beginning in the cascade
- **THEN** a scored run begins and stroke judgments accumulate exactly as note
  judgments do for a keyboard score
...
window scores highest. For a **percussion** score, whether an attack is an attack
*of that note* SHALL be decided by the stroke identity of `music-drum-scoring` —
kit-piece-grain equivalence with the open/closed hi-hat shading — in place of
pitch equality; the verdict scale, the two mode metrics and the windows are
unchanged. The measured quantity SHALL depend on the mode:
...
#### Scenario: A stroke of the right piece binds like a press of the right pitch

- **WHEN** a percussion onset expects a snare and the player strikes any snare
  number within the timing window
- **THEN** the stroke binds to that onset and receives a timing verdict on the
  same scale a keyboard press would
...
any onset. For a **percussion** score the same rule reads over stroke identity: a
stroke of a **piece** that no open onset requires is the extra/wrong note. A wrong
note SHALL reduce the run's correctness dimension but SHALL NOT
...
#### Scenario: A stroke of an unrequired piece is an extra note

- **WHEN** the player strikes a piece that no currently-open percussion onset
  expects
- **THEN** the stroke is recorded as an extra note and lowers the correctness
  dimension, and playback continues unaffected

### Requirement: Sustain Judgment

For each correctly-attacked note of a **keyboard** score, scoring SHALL evaluate how long
the pitch was held relative to the note's intended duration and SHALL derive a sustain
ratio bounded to a
...
For a **percussion** score the sustain dimension SHALL NOT be evaluated at all: a
stroke is an attack, and its note-off timing is a property of the hardware, not
of the player (`music-drum-scoring`). No percussion stroke SHALL ever carry a
sustain ratio or a sustain penalty.
...
#### Scenario: A drum stroke's release is never judged

- **WHEN** a percussion stroke binds correctly and is released at any moment
- **THEN** no sustain ratio is derived and no sustain penalty is applied
...
for a **percussion** run, the timing and correctness dimensions only, with their
weights renormalized to preserve the keyboard blend's relative ratio
(`music-drum-scoring`).
...
#### Scenario: A percussion run's percentage blends two dimensions

- **WHEN** a percussion run's live percentage is computed
- **THEN** it blends timing and correctness only, and a run of perfectly-timed,
  correct strokes can reach 100 with no sustain contribution
...
A sub-score for a mode with no onsets SHALL be absent (not zero). The stamping,
classification and sub-scores SHALL be instrument-agnostic: a percussion run
classifies and sub-scores by the same rules, over its two-dimension blend.
...
aggregates (timing, correctness, sustain — the sustain aggregate and the per-note
sustain fields being **absent, not zero**, for a percussion run, exactly as an
absent mode sub-score is), the mean signed free-run timing offset and the mean
Wait-Mode reaction time (each absent when that mode had no hits), the count of onsets by
...
#### Scenario: A percussion record carries no sustain fields

- **WHEN** a percussion run finalizes
- **THEN** its record's sustain aggregate and per-note sustain values are absent,
  not zero, and deserializing the record preserves that absence
