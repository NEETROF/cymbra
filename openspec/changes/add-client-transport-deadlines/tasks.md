...
- [x] 1.1 Add `apps/music/lib/services/rpc_deadlines.dart` with the budget constants
...
- [x] 1.2 Populate the override table — `transfer`: `GetCatalogScoreBytes`,
...
- [x] 1.3 Comment the table with the known trap from design D2: an RPC added later
      inherits `interactive`, so a new long-running RPC must be added here or it will
      time out on first use.
- [x] 1.4 Add `RpcDeadlines implements ClientInterceptor` in the same file,
...
- [x] 1.5 Override `interceptStreaming` the same way, so the policy holds if a
      streaming RPC is ever added (the app has none today).
- [x] 1.6 Expose a `keepAlive` `rpcDeadlinesProvider` returning the shared instance
      (it is stateless, so one instance serves every adapter).

## 2. Wire the interceptor and the channel options

- [x] 2.1 `grpc_client.dart`: add `connectTimeout: kConnectTimeout` (10 s) to
...
- [x] 2.3 Give each adapter an injected `RpcDeadlines` and pass it as
...
- [x] 2.4 Wire the interceptor into the `AuthServiceClient` that `tokenRefresher`
      builds directly (`grpc_client.dart:398`) — it bypasses every adapter, and an
      unbounded refresh there means a hung sign-in.
- [x] 2.5 Update the adapter providers to read `rpcDeadlinesProvider`, and run
      `dart run build_runner build --delete-conflicting-outputs`.
- [x] 2.6 Grep `lib/` for any remaining generated-client construction not covered by
      2.3/2.4 and wire it too.

## 3. Status-code classification (must land in the same commit as section 2)

- [x] 3.1 `auth_service.dart`: add `case 4: return AuthError.unavailable;` to
...
- [x] 4.2 The helper MUST own an explicit `StreamSubscription` and cancel it in a
      `finally` — `onlineStatus` is a broadcast stream, so an unsatisfied listener
      leaks one subscription per score open.
- [x] 4.3 The helper MUST listen for the transition only, never re-read the current
      value, so it cannot double-fire with the pre-flight check of 4.5.
- [x] 4.4 Apply the race to the two network waits in `_load`: `_fetchScoreBytes` on
...
- [x] 4.5 Add the pre-flight `isOnline()` gate to the cache-**miss** path, mirroring
...
- [x] 4.6 Do **not** roll the race out to the other screens (`community`, `profile`,
...
- [x] 5.1 `open_score.dart`: add a cancel affordance to the progress dialog. Keep
      `barrierDismissible: false` (an accidental barrier tap should not cancel a load)
      — the exit is the explicit control.
- [x] 5.2 On cancel: clear `selectedScoreProvider` (the existing
...
- [x] 5.3 The cancel control must **not** pop the dialog itself — the main flow
...
- [x] 5.4 Cancelling shows **no** error banner and returns the user to the previous
      screen — it is a user decision, not a failure.
- [x] 5.5 Add the localized cancel label to the ARB files and regenerate l10n.
- [x] 5.6 Make sure `sub.close()` still runs on the cancel path (today it only runs
      via the loaded/failed branches).

## 6. Abandon the upload on leave (design D11)

- [x] 6.1 `score_upload_notifier.dart`: add a disposal flag set from `ref.onDispose`
...
- [x] 6.2 Do **not** add a `PopScope` to `ScoreUploadScreen`. Leaving stays free; the
      exit must never wait on a network answer (design D11).
- [x] 6.3 Show **no** message on leave — not "cancelled", not "failed", not "sent".
      The app cannot know which. `MyUploads` already refreshes on the `result`
      transition and reports the truth.
- [x] 6.4 Reword the `AlreadyExists` case in `uploadErrorMessage` as a statement of
      ...
      strings.
- [x] 6.5 Confirm the dedup claim before relying on it: `(owner_id, sha256)` UNIQUE
...
- [x] 7.1 `private_soundfont_service.dart`: `interactive` timeout on `list`,
...
- [x] 7.2 `soundfont_source.dart`: font-bytes download is `bulk` — uncapped, same
      comment.
- [x] 7.3 `soundfont_preview_service.dart` and `score_preview_service.dart`: bounded
      media fetches — `transfer` timeout.
- [x] 7.4 Make sure a timeout surfaces as the seam's own exception type
      (`PrivateSoundFontException` etc.), not a raw `TimeoutException`, so no raw
      technical text can reach the UI.
- [x] 7.5 `imported_soundfonts.dart` `importSoundFont()`: persist the local registry
...
- [x] 8.1 `test/services/rpc_deadlines_test.dart` — table test over
      `deadlineForMethod`: every override path returns its category budget, and an
      unknown path returns `interactive`.
- [x] 8.2 Assert the interceptor actually attaches the deadline: call
...
- [x] 8.3 Assert the merge direction (design D3): an explicit per-call
...
- [x] 8.4 Per-adapter wiring test: for each of the 17 clients from 2.3/2.4, assert the
...
- [x] 8.5 Regression test for the offline fallback: a fetch failing with
...
- [x] 8.6 Test that a timed-out refresh is classified transient and leaves the stored
      session intact (extend `test/services/token_refresher_test.dart`).
- [x] 8.7 Race test: with a never-completing fetch, emit `false` on a controllable
...
- [x] 8.8 Leak test: on a normal load with no connectivity transition, assert the
      connectivity subscription is cancelled (a fake `ConnectivityService` counting
      listen/cancel).
- [x] 8.9 Test that a late result from an abandoned load — arriving after cancel or
      after an offline abort — does not overwrite state.
- [x] 8.10 Pre-flight test: offline at load time produces the offline outcome with the
      catalog service never called (verify with a strict mock).
- [x] 8.11 Widget test on `open_score`: the progress dialog exposes a cancel control,
      and cancelling pops it, clears the selection and shows no error banner.
- [x] 8.12 HTTP seam tests: a control call times out and surfaces the seam's own
      exception; a bulk call is issued with no wall-clock timeout.
- [x] 8.13 Upload-abandon test: dispose the notifier mid-`submit()` and assert the
      late completion writes no state and throws nothing, in both debug and release
      semantics (the flag, not the incidental drop).
- [x] 8.14 Assert leaving mid-upload surfaces no message at all, and that a subsequent
      `AlreadyExists` is rendered as a fact rather than an error.
- [x] 8.15 `flutter test --coverage --exclude-tags golden` and confirm the gate stays
      ≥ 80%.
...
- [x] 9.10 `melos run analyze`, `dart format`, and `dart run custom_lint` clean.
- [x] 9.11 `openspec validate add-client-transport-deadlines --strict` passes.
