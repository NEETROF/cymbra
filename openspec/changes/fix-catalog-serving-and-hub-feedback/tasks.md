...
- [x] 6.4 MANUAL / live env (A): smoke crawl with the new mount (`LIMIT=5 docker compose ... up`) → `.mxl` appears under `SCORES_DIR/<prefix>/<shard>/<uuid>.mxl`; open a freshly crawled score in the hub **without** running `sync-scores.sh` → it plays; then run `sync-scores.sh` → S3 mirror OK. (Requires the prod box / crawler images — not runnable in this session.)
- [x] 6.5 MANUAL / device (B): with the backend unreachable, tapping a catalog score shows a spinner then an error banner in Synthesia (no silent blank). (Covered by `player_load_feedback_test.dart` at the widget level; end-to-end check needs a running app.)
