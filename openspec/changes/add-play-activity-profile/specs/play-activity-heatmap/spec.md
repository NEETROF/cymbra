...
average success rate** (the color weighting requested), and the day's **play count** SHALL
be conveyed by the cell (intensity/size and a tooltip showing count and exact success
...
#### Scenario: Cell color reflects the day's success rate

- **WHEN** a day has recorded sessions
- **THEN** its cell is colored according to that day's average success rate

#### Scenario: Count is conveyed alongside color

- **WHEN** a user inspects a played day's cell
- **THEN** the number of songs played that day (and the exact success percentage) is available via the cell's intensity and tooltip
