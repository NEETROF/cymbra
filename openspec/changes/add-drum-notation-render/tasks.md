...
- [ ] 1.1 Verify the Bravura assets **first**: `unpitchedPercussionClef1`
...
- [ ] 1.2 Derive the engraved head class in `crates/musicxml-core` beside the
...
- [ ] 1.3 Crate tests: the cymbal set classifies as x, the drums as oval, 46 as
      open, unresolved as oval; the field serializes under the `serde` feature
      (the wasm JSON contract)
- [ ] 1.4 Regenerate the app bridge (`flutter_rust_bridge_codegen generate` —
...
- [ ] 2.1 `smufl.ts`: percussion clef and x-head glyphs; give the percussion
      ...
      would catch it
- [ ] 2.2 `painter.ts`: retire the `percussion_unsupported` result and the
...
- [ ] 2.3 Percussion paint path: single staff, percussion clef, **no** armature
...
- [ ] 2.4 Heads from the crate's head class — x forms following the duration
...
- [ ] 2.5 Retire the unpreviewable state's consumers: `ScorePreview.vue`,
...
- [ ] 2.6 Vitest over the real fixtures (`groove_ouvert` at minimum): SVG
...
- [ ] 3.1 `notation_playback.dart`: plumb `voice` onto `TimedRest` (TimedNote
      has it since `add-drum-kit-view` 1.6; rests do not yet), so the Staff
      painter can displace rests by voice
- [ ] 3.2 `staff_painter.dart`: percussion clef glyph in `smufl.dart` (same
...
- [ ] 3.3 Heads from the bridged head class (x forms by duration class, the
      ...
      unchanged
- [ ] 3.4 Widget tests on the fixture scores: percussion clef drawn, snare/kick
...
- [ ] 3.5 Confirm the StaffPainter reuse surfaces inherit the path — the
...
- [ ] 4.1 `partition_painter.dart`: the percussion paint path — single
...
- [ ] 4.2 Scope the staff-collapse rule to keyboard scores: for percussion the
...
- [ ] 4.3 Hands/feet colours on engraved heads, playhead emphasis and the
      Paper theme's darkened palette applying unchanged
- [ ] 4.4 Both `PartitionPainter` instantiation sites of the Partition screen
      ...
      written measures
- [ ] 4.5 Tests: fixture-driven paint assertions as in §3.4, plus a
      `fifths ≠ 0` synthetic percussion score drawing no armature, and a
      measure-select smoke test on a percussion score

## 5. Mode re-offer

- [ ] 5.1 Lift the toggle restriction (`player_screen.dart:940`): a percussion
      ...
      **default** on load
- [ ] 5.2 Wait Mode stays not-offered for a percussion score — untouched here,
      asserted by an existing-behaviour test so this change cannot regress the
      `add-drum-scoring` interim
- [ ] 5.3 Tests: toggle offers cascade/Staff/Partition for percussion and the
...
- [ ] 6.1 App test pinning the two tables' overlap: every GM number
...
- [ ] 6.2 Parity check as a test artifact: the same fixture rendered by the
...
- [ ] 7.3 `melos run analyze`, `dart format`, `dart run custom_lint` clean
- [ ] 7.4 `cd apps/music && flutter test --coverage --exclude-tags golden`, coverage ≥ 80%
- [ ] 7.5 BO — `yarn test` and the Playwright e2e (pass `BO_E2E_PORT` to avoid
      colliding with another worktree's dev server)
- [ ] 7.6 `flutter_rust_bridge_codegen generate` ran (crate public API changed)
      and the regenerated bridge is committed; `yarn gen:wasm` documented as a
      local step (the wasm itself is not committed)
- [ ] 7.7 `openspec validate add-drum-notation-render --strict`
