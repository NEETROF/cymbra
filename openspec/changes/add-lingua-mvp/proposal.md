...
Ce premier change livre un produit **entièrement local, sans compte ni backend** : la valeur se valide sur l'utilisateur fondateur avant tout investissement serveur. La sync Cymbra ID (audience `lingua`), les ports navigateurs, l'app mobile et le catalogue de livres sont des changes ultérieurs déjà explorés — avec une contrainte de séquencement assumée : **l'utilisateur fondateur vit sur Safari macOS/iOS au quotidien**, donc le port Safari est le premier follow-up (dogfooding quotidien en dev-mode dès que le MVP Chrome s'auto-valide), avant l'élargissement des canaux.
...
- **Hors périmètre (changes ultérieurs déjà explorés, dans cet ordre)** : ① **port Safari macOS** (extension convertie, analyse native via nativeMessaging — dogfooding quotidien du fondateur en dev-mode) ; ② backend `lingua` + audience Cymbra ID + sync multi-device ; ③ **app conteneur iOS + extension Safari iOS** (dogfoodée via TestFlight interne, utile une fois la sync là) ; ④ **port Firefox** (desktop + Android, même zip) ; puis app browser mobile, catalogue de livres/TextProfile communautaire, OCR/capture d'image, adaptateurs Codex/Aider, packs langues romanes. **Navigateurs supportés à terme : Chrome (+ Edge desktop, même artefact), Safari, Firefox** — les canaux « tier 3 » (Edge Canary Android par ID, Edge/Samsung stores curés, forks Chromium) sont explicitement **non supportés** : le zip standard peut y tourner, rien n'y est promis ni testé.
