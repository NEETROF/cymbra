...
Ce premier change livre un produit **entièrement local, sans compte ni backend** : la valeur se valide sur l'utilisateur fondateur avant tout investissement serveur. La sync Cymbra ID (audience `lingua`), les ports navigateurs, l'app mobile et le catalogue de livres sont des changes ultérieurs déjà explorés — avec un fait de séquencement heureux : **le fondateur navigue sous Chrome sur macOS — le MVP est donc son dogfooding quotidien dès le premier jour** — et sous Firefox sur iOS, où aucun navigateur tiers ne peut porter d'extension (règle Apple : Safari uniquement) : ce trou iOS est assumé pour le MVP, comblé plus tard par le port Safari (canal du grand public Apple) puis l'app browser.
...
- **Hors périmètre (changes ultérieurs déjà explorés, dans cet ordre)** : ① **port Firefox** (le moins cher ; desktop + Android en bonus gratuit, même zip — le canal des enthousiastes) ; ② backend `lingua` + audience Cymbra ID + sync multi-device ; ③ **port Safari macOS + app conteneur iOS + extension Safari iOS** (le canal du grand public Apple — seule voie extension sur iOS, où Firefox/Chrome sont des habillages WebKit sans extensions) ; puis app browser mobile (qui servira aussi l'iOS du fondateur), catalogue de livres/TextProfile communautaire, OCR/capture d'image, adaptateurs Codex/Aider, packs langues romanes. **Navigateurs supportés à terme : Chrome (+ Edge desktop, même artefact), Safari, Firefox** — les canaux « tier 3 » (Edge Canary Android par ID, Edge/Samsung stores curés, forks Chromium) sont explicitement **non supportés** : le zip standard peut y tourner, rien n'y est promis ni testé.
