...
### D12 — Matrice navigateurs : un artefact, trois variantes de build
Une seule source WebExtension MV3 ; le build produit trois variantes de manifest (chromium / firefox / safari). Ce qui varie est confiné derrière deux coutures : l'**`AnalyzerPort`** (Chrome/Edge = WASM dans le content script ; Firefox = WASM dans l'event page — sa CSP bloque le WASM en content script, spike jour 1 ; Safari = **aucun WASM** : nativeMessaging vers le handler natif de l'app conteneur, qui linke `lingua-core` compilé ARM) et la **surface de panneau** (Side Panel API sur Chromium ; `sidebar_action` sur Firefox ; drawer injecté seul sur Safari, qui n'a pas d'API de panneau). Firefox : `background.scripts` (event page) déclaré à côté du `service_worker`, host permissions optionnelles à l'install (prompt), même zip publié desktop + Android sur AMO. Les canaux tier 3 ne reçoivent ni test ni promesse.

### D13 — App conteneur Apple : une fiche, deux OS, l'analyse en natif
...
- [WASM dans les content scripts Firefox (CSP)] → spike jour 1 du port ; le repli est déjà le design (WASM en event page + messages).
...
- [4 navigateurs pour un solo dev] → un artefact unique + coutures (D12) ; l'ordre d'implémentation reste Chromium → Firefox → Apple ; matrice de parcours manuel par navigateur avant chaque release.
