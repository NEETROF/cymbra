...
- A new multi-step **contribution screen** in the app, reached only when signed
  ...
  1. **Upload** — pick a `.musicxml`/`.xml` file or a zipped `.mxl`; the user MUST
     declare a **rights basis** — author or public domain / free licence — and tick
     the **CGU confirmation checkbox** before the file can be submitted.
...
- **Durable storage**: validated score bytes are stored in the object store
  ...
  creation date, chosen difficulty, and the rights attestation (declared basis +
  confirmation).
...
- `score-upload`: App-side, multi-step contribution flow for a signed-in user —
  file picking (plain + zipped MusicXML), the mandatory rights-attestation CGU gate,
...
- `backend-score-storage`: Backend (gRPC) surface that receives an uploaded
  ...
  records ownership + upload date + difficulty + rights attestation in
...
- **Backend (`backend/`)**: a new gRPC service + `.proto`, server-side MusicXML
  ...
  scores table (owner FK, object key, difficulty, rights basis + confirmation, timestamps),
