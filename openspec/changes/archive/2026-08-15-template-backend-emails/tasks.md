...
- [x] 1.3a (external design/site dependency, non-blocking) Produce the neutral **Cymbra ID** logo = the Music app icon reduced to the **"C"** with the audio motif removed; add it to the **`cymbra.app`** site at a stable URL; then wire `<img alt="Cymbra ID">` above the text wordmark fallback. Until then the wordmark ships alone. Done: the mark was rebuilt as vector art and lives at `https://cymbra.app/brand/cymbra-id.png` (96×96, NEETROF/cymbra-site#3, which also replaced the Astro placeholder favicons with it). The C is scaled +15% vs the Music icon so it stays legible at favicon sizes; `CYMBRA_EMAIL_LOGO_URL` is now set in both env examples.
