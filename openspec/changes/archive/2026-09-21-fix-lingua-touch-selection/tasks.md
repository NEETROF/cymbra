...
- [x] 5.6 Desktop Chromium and Firefox: a mouse drag still opens the expression card with no perceptible delay, a plain click on a painted word and Alt-click on a non-painted one are unchanged, and the keyboard shortcut still works. Done 2026-09-21 on both, loaded from `dist-firefox`/`dist-chromium` — the mouse path shows no delay, confirming the pointer lift flushes the debounce rather than waiting it out.
- [x] 5.7 Confirm the debounce value on device and adjust the constant if 350 ms reads as laggy or as too eager (design's open question). Confirmed 2026-09-21 across the Firefox Android and iPhone passes — not bothersome, so 350 ms / 1200 ms held stand unchanged.
