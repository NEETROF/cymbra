...
- [x] 6.1 Once deployed, close and reopen a real campaign from the console, so both controls are exercised on a shipped build rather than on a dev server. The manual `UPDATE plans.beta_campaigns SET closed_at = NULL` survives only in this change's own documents, where it explains why the change exists — there is no runbook to prune — VALIDÉ 2026-08-25 sur `midi-drums` en production (backend 0.21.1). Les deux contrôles ont bien agi : `closed_at` est repassé à NULL et **l'unique adhésion est redevenue active**, ce qui est la preuve observable que rouvrir rend l'accès aux inscrits sans toucher aux adhésions. À noter, et c'est le comportement voulu : `enrollment_closes_at` est resté posé après la réouverture de la campagne — rouvrir les inscriptions est un geste séparé (`ReopenEnrollment`), donc aucun nouveau code n'est rachetable tant qu'on ne l'actionne pas.
