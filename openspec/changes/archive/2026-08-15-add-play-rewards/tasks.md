...
- [x] 1.1 Create `backend/music/src/play_rewards_core.rs` with `PlayRewardConfig`: the
...
- [x] 1.2 Implement `fn performance_award(accuracy, times_already_paid, level, already_today,
...
- [x] 1.3 Implement `fn practice_award(already_awarded_today, cfg) -> i64` — the flat amount
      once, zero thereafter.
- [x] 1.4 Reuse the leaderboard's difficulty weighting rather than a second scale: read the
...
- [x] 1.5 Unit-test 1.2 and 1.3 exhaustively: floor boundary (just below / exactly at), each
...
- [x] 1.6 Add a test that states the anti-grind property directly: replaying one piece all
      day earns strictly less than playing the same number of distinct pieces, and never
      exceeds the daily cap.

## 2. Ledger + repo

- [x] 2.1 Migration: extend the `award_kind` CHECK on `music.curation_points` with
...
- [x] 2.2 Add `AwardKind::Performance` / `AwardKind::Practice` with their persisted strings.
- [x] 2.3 Extend `CurationRewardsRepo`: append an award carrying an `award_key` (returning
...
- [x] 2.4 Verify each new query is served by an existing index (`curation_points_user_idx`,
      plus the new partial unique index for the per-piece count); note it in the migration if
      one has to be added.
- [x] 2.5 Repo tests on the fake: the same `award_key` inserts once, two different keys both
      insert, and the per-piece count only counts `performance` rows.

## 3. Awarding module

- [x] 3.1 Extend `CurationRewardsSink` with `award_performance(user, piece, accuracy, level,
      session_id)` and `award_practice(user, local_day)`, both returning the points actually
      awarded (0 = gated out / capped / already paid).
- [x] 3.2 Implement them on `CurationRewardsModule` over the pure math + repo, keyed on the
      `award_key` (session id / local day) so a retry is a no-op.
- [x] 3.3 Grant any newly-due badge after a play award, the way the rating paths already do —
      a level or milestone crossed by playing must not wait for the next profile read.
- [x] 3.4 Module tests against the fake: a good run pays; the same session id replayed pays
...
- [x] 4.1 Call the two sink methods from `PlayGrpc` where `record_engagement` is already
      called, so `PlayModule` stays unaware of rewards.
- [x] 4.2 Resolve the piece's catalog difficulty through the already-wired
      `CatalogSearchRepo` (the same seam that tells a catalog score from an upload today); an
      unresolvable piece weighs neutral.
- [x] 4.3 Make the award **best-effort with respect to the ack**: a failure logs and is
      swallowed, the session is still stored and acknowledged. Add a test proving a failing
      award does not fail the ingest.
- [x] 4.4 Derive the practice award's local day from `practiced_at` + `tz_offset_minutes`,
      reusing `play_core::local_day` rather than a second implementation.

## 5. API

- [x] 5.1 Add `points_awarded` to `RecordPlaySessionResponse` and `RecordPracticeResponse`
      (additive fields on existing responses — no new RPC).
- [x] 5.2 Populate them from the sink's return value; a sink that is not wired reports 0.
- [x] 5.3 Handler tests: a paying session reports its amount, a sub-floor session reports 0,
      a replayed session reports 0 the second time.
- [x] 5.4 Regenerate the Dart gRPC stubs (`melos run gen-grpc`).

## 6. App

- [x] 6.1 Carry `pointsAwarded` through the play-sync outbox path into the session-summary
      state (it arrives on the ack the client already awaits).
- [x] 6.2 Show the "+N" cue on the session summary when the amount is positive, and show
      nothing when it is zero — reusing the rating action's existing cue treatment.
- [x] 6.3 Reuse the existing reward-celebration path for a level crossed by playing.
- [x] 6.4 Add the l10n keys for the cue to all supported locales.
- [x] 6.5 Widget tests: cue shown for a positive amount, absent for zero, and the summary
      otherwise unchanged.
- [x] 6.6 Notifier tests with mockito-generated mocks via `ProviderScope` overrides for the
