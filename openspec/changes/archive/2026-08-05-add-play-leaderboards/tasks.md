...
- [x] 6.4 Tapping a listed player (or own standing) opens their profile (`ProfileScreen`). Security: the profile read is already gated **server-side, fail-closed** (`user` module `get_player_profile`: viewer from the token, non-owner sees only a public+eligible profile, else `NotFound`) — a modified client cannot view a non-public profile. Own profile always viewable. No new backend needed; covered by existing `private_profile_is_not_found_to_others` tests.
