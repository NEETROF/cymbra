...
- **Beta access is claimed on Discord, never distributed as a list of codes**: a `/beta`
  ...
  capability (`music-access-codes`, already shipped) to mint a
...
### New Capabilities
- `platform-discord-announcements`: backend-driven publication of product events to a Discord server —
  ...
  kill-switch.
- `platform-discord-bot-actions`: the Cymbra Discord application — signature-verified interactions
...
- `music-community-invite`: the flag-gated in-app entry point to the community, resolved
...
- **External dependency**: Discord's API and its rate limits (~5 requests/second per webhook,
  ...
  below them.
- **Consumed, not built**: the access-code issuing port `AccessCodeIssuer` is **already on
  `main`** (`backend/plans/src/ports.rs`, from `add-premium-subscription`) and its doc-comment
  names Discord `/beta` as an intended issuer, so this change wires it rather than defining it.
  A campaign refusing enrolment surfaces as "beta not open" from the port's own error, and the
