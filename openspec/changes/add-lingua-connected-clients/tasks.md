...
- [ ] 4.1 Host app sign-in sheet (SwiftUI, Cymbra styling, French copy): Sign in with Apple first (`ASAuthorizationController`) and Continue with Google (`ASWebAuthenticationSession` with PKCE on the iOS OAuth client, no SDK), opened through the app's URL scheme with the requested provider; on success the id_token is written once to the App Group and the reader is told to go back to Safari; a cancel shows nothing, a failure is worded per provider, never as a password error
