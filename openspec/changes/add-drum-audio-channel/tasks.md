...
- [x] 1.1 Export the two channel constants (melodic 0, drum 9) beside
  ...
  definition
- [x] 1.2 Test that pins the values (0 and 9) so a refactor cannot silently move
  them — the constants are wire-like: three synth sites and every stored kit
  font depend on them

## 2. App engine (`apps/music/rust/src/api/`)

- [x] 2.1 `audio_core.rs`: `AudioEvent::NoteOn`/`NoteOff` gain the channel
...
- [x] 2.2 `renderer.rs`: route each event to its channel via the shared
  constants; `ReplaceSynth` keeps silencing both channels across the swap
- [x] 2.3 `audio.rs`: FFI entry points `drum_on(key, velocity)` /
  `drum_off(key)` beside the melodic pair; `note_on`/`note_off` byte-for-byte
  untouched
- [x] 2.4 `audio.rs`: the SoundFont swap gains an awaitable completion (resolves
...
- [x] 2.5 Engine helper exposing a local `.sf2`'s family evidence from its
  preset headers (has bank-128 presets / has melodic presets), for the app's
  import detection — pure parse, host-testable
- [x] 2.6 `flutter_rust_bridge_codegen generate` (public API change)

## 3. Console renderer and back office

- [x] 3.1 `crates/audio-wasm/src/lib.rs`: delete the local `PIANO_CHANNEL` copy
...
- [x] 3.2 `yarn gen:wasm` and commit nothing stale — the console must run the
  rebuilt wasm (see the back-office wasm-rebuild gotcha)
- [x] 3.3 `useSoundFontChoice.ts`: carry each option's family from the listing;
...
- [x] 3.4 `ScoreDetailView.vue`: lift the Play guard — Play for a percussion row
...
- [x] 3.5 Localised "no drum kit available" state (fr/en) when the catalog holds
  no accepted percussion-family font — distinct from error states
- [x] 3.6 `SoundFontDrawer.vue`: the create drawer offers both families
...
- [x] 4.1 Migration on `music.soundfonts`: `UPDATE instrument 'piano' →
  'keyboard'`, default `keyboard`, CHECK on (`keyboard`,`percussion`); nothing
  else in the migration
- [x] 4.2 `backend/server/src/soundfont.rs`: normalise legacy `piano` (and
...
- [x] 4.3 Preset-bank verification helper (pure, host-testable): parse the
...
- [x] 4.4 Wire the verification into admin upload, private-library import sync,
...
- [x] 4.5 One-shot ops pass over existing `music.soundfonts` rows: re-read each
...
- [x] 5.1 `soundfont_synth.rs`: the render takes the channel (or family) instead
  of hardcoding `PREVIEW_CHANNEL = 0`; consume the shared constants
- [x] 5.2 Registry: declare `catalog.preview.drum_soundfont_id` beside
...
- [x] 5.3 `score_preview_module.rs`: remove the percussion skip; choose the font
...
- [x] 5.4 Confirm the acceptance path enqueues percussion renders again (the
...
- [x] 5.5 `soundfont_preview.rs`: a second fixed sample sequence — a short
...
- [x] 7.1 `soundfont_catalog_service.dart`: map the wire `SoundFont.instrument`
  onto the catalog entries (the field exists; the app drops it today)
- [x] 7.2 `piano_catalog.dart`: entries carry the family; imported entries
  persist the detected family in the registry JSON
- [x] 7.3 Per-family persisted selection: the existing `selected_piano` key
...
- [x] 7.4 Font-follows-score: on player open, resolve the loaded score's family
...
- [x] 7.5 Percussion readiness: the player treats a percussion score as ready to
...
- [x] 7.6 `player_notifier.dart`: route scheduled notes by the score's family —
...
- [x] 7.7 `sound_selector_field.dart` + the player settings drawer: offer the
  loaded score's family only; label the percussion picker as the kit picker
  (fr/en)
- [x] 7.8 `soundfonts_screen.dart`: show every font with a family badge
  (no filtering — the screen is not scoped to a score), localised fr/en
- [x] 7.9 `soundfont_importer.dart`: detect the family via the engine helper
...
- [x] 8.1 `cargo fmt --all --check` and `cargo clippy --workspace --all-targets
  -- -D warnings`
- [x] 8.2 `cargo llvm-cov --workspace --fail-under-lines 80` with the repo's
...
- [x] 8.4 `cd apps/music && flutter test --coverage --exclude-tags golden`,
  coverage ≥ 80%
- [x] 8.5 BO — `yarn test` and the Playwright e2e (pass `BO_E2E_PORT` to avoid
  colliding with another worktree's dev server)
- [x] 8.6 `flutter_rust_bridge_codegen generate` ran (2.6) and the generated
  bridge is in sync
- [x] 8.7 `openspec validate add-drum-audio-channel --strict`
