...
- [ ] 2.4 Implement the hybrid re-review flag: when `rating_count ≥ N` and `avg ≤ T` (config; defaults N=5, T=2.0 on the 1–5 scale), mark the score eligible for re-review without touching `moderation_status`. Effective value per rating: explicit stars, else verdict-implied (dislike≈1.5 / like≈3.5 / love≈5).
...
- [ ] 3.2 Implement the swipe interaction (vetted card-swiper package OR custom `GestureDetector`/`AnimatedBuilder`) with left=dislike / right=like / up=love, reusing `ScoreCard` for the card visual.
- [ ] 3.3 Add on-screen dislike/like/love buttons that perform the identical actions (accessibility parity), plus a Skip control that advances without recording a rating.
...
- [ ] 6.2 Flutter: notifier records verdict/stars via a fake rating service; buttons mirror swipes; Skip advances without recording; deck shows only accepted cards and empties correctly; preview fires no scoring. `flutter test --coverage` ≥ 80%.
