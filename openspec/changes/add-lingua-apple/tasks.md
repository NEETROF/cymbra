...
- [ ] 1.1 `build.mjs`: `safari` target derived from the firefox manifest (`background: { scripts, persistent: false }`, no `browser_specific_settings`, no `sidePanel`/`identity` permissions, static content script) → `dist-safari/`; `yarn build:safari`
- [ ] 1.2 Replace the `__TARGET__ === "firefox"` checks with named capability predicates true for firefox and safari (event-page engine in `create-port.ts`, in-page drawer routing in `content.ts` and `popup.ts`, static injection in `background.ts`, shortcut targets)
- [ ] 1.3 Icon set in the base manifest (every variant), sized for the toolbar, the extension list and the converter's app icon
- [ ] 1.4 `lingua-extension-check` builds the safari variant and greps the bundle for the expected per-target code

## 2. Reading on long pages (every variant)

- [ ] 2.1 Viewport-windowed painting in `reading/highlight.ts` (`IntersectionObserver` on block containers, `rootMargin: "100% 0px"`, rAF-batched repaint, full paint without the observer) and `ResolvedToken.container` in `reading/scan.ts`; unit tests for window membership, repaint on intersection change and the fallback
...
- [ ] 3.1 `state/platform.ts` `hasShortcutEditor()` (`pointer: coarse`); hide the shortcut-editor link in the popup and in `mountSettings`; tests
- [ ] 3.2 Full-width popup under `(pointer: coarse)`

## 4. First run and sign-in on Safari

- [ ] 4.1 With no declared level, the in-page pastille offers the level choice (the existing settings module in the drawer)
- [ ] 4.2 Provider buttons that need `identity.launchWebAuthFlow` are shown only where it exists (feature detection, aligned with `add-lingua-account-parity`); email/password on Safari
