...
- FROM: `### Requirement: Three-State Key Feedback`
  TO: `### Requirement: Key And Pad Feedback States`
...
For a **percussion** score the pad strip SHALL be playable the same way, at
all the same times: a pointer-down on a pad SHALL produce a note-on for one
of the General MIDI numbers its lane collapses, and the matching pointer-up
(or pointer-cancel) the corresponding note-off, through the same entry points
(see `music-drum-input`) — so a pad tap drives feedback, and, once
...
the strip now emits, and the former "a pad tap does nothing yet" scenario is
deliberately replaced by the input scenarios below.

The number a pad emits SHALL be deterministic: each named piece's members
...
The **kick pedal** SHALL be playable like a pad and SHALL emit the kick by
...
The strip SHALL offer **no gesture that distinguishes an open from a closed
hi-hat stroke**: it presents one hi-hat pad, emitting its lane's first present
...
pedal-controlled on the instrument, not a second aim point — a split pad
...
#### Scenario: The kick pedal emits the kick
- **WHEN** the user taps the kick pedal
- **THEN** a note-on for General MIDI 36 is emitted — or 35 when the score
  writes its kick only as 35 — and the pedal flashes
...
#### Scenario: Pads playable while stopped
...
For a **percussion** score the same mapping SHALL run against the pad layout,
and the whole strip SHALL be live: a pointer anywhere in the pads band hits
the lane whose horizontal span contains it, and a pointer anywhere in the
pedal band hits the kick — the visual insets between pads are styling, not
hit boundaries. A drummer at tempo aims at a region, not a rounded rectangle;
a tap swallowed by a decorative gutter is a ghost stroke, which reads as
broken input.
...
#### Scenario: A tap in the gutter still strikes
- **WHEN** the pointer lands between two pads' drawn rectangles
- **THEN** the hit-test returns the lane whose horizontal span contains the
  pointer — no dead zone exists on the strip

#### Scenario: The pedal band is all kick
- **WHEN** the pointer lands anywhere in the pedal band
- **THEN** the hit-test returns the kick

### Requirement: Key And Pad Feedback States
...
For a **percussion** score the pads and the kick pedal SHALL carry exactly one
feedback state: a brief, time-based **struck** flash on the surface whose
piece was struck — by an on-screen tap or by an external stroke resolving to
...
the controller only: the cascade's falling notes do not react to strokes, the
same division the keyboard makes between its keys and the waterfall.
...
#### Scenario: A struck pad flashes
- **WHEN** a pad is tapped, or an external stroke resolves to its lane
- **THEN** the pad shows the struck flash, which decays on its own
...
#### Scenario: The pedal flashes on kick strokes
- **WHEN** a kick stroke arrives from the pedal or an external kit
- **THEN** the pedal shows the struck flash
...
The pad strip SHALL support the same simultaneity: each pointer maps
independently to the pad (or pedal) it lands on, and simultaneous strokes on
different surfaces each emit. **Every pointer-down on a pad SHALL be a fresh
stroke, even while another pointer still rests on the same pad** —
alternating two fingers on one pad is how a roll is played; one-shots have no
...
#### Scenario: Two pads struck together
- **WHEN** two pointers strike two different pads at the same time
- **THEN** both strokes are emitted and both pieces sound

#### Scenario: A two-finger roll on one pad
- **WHEN** two fingers alternate rapidly on a single pad, each pressing while
