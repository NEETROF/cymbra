...
- [ ] 1.3 A flags/config **service** with typed accessors and a two-tier cache: **L1** = an in-process snapshot of the whole flag/config set (hot-path reads, near-zero latency), refreshed atomically on a short TTL (~15 s safety net) or on invalidation. **L2 invalidation** — prefer **RESP3 client-side caching** (`CLIENT TRACKING` BCAST on a `flags:` prefix, flags stored as Redis keys populated from Postgres, on a **dedicated tracking connection**) if the Rust Redis client supports it cleanly, so L1 is auto-invalidated with no hand-rolled wiring; **else fall back to manual Redis pub/sub** (coarse "reload" is enough given the whole-snapshot L1). Fail-safe: Redis down → L1 last snapshot + TTL-poll Postgres; Postgres down → code defaults (kill-switches safe). Worker instances share the same service.
- [ ] 1.3a Spike/verify the chosen Rust Redis client's RESP3 client-side-caching support (e.g. `fred` tracking vs `redis-rs` push handling) and the dedicated-tracking-connection wiring; pick client tracking or the pub/sub fallback based on it.
