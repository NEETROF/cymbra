...
The state-management capability defines how Cymbra's app UI state is structured
and wired: state is exposed through Riverpod 2 providers (codegen), modeled as
immutable Freezed types, and external dependencies are surfaced as overridable
providers so tests can inject fakes without constructor injection. Generated
sources are reproducible from `build_runner` and excluded from version control.
