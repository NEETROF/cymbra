...
A subscriber SHALL have **unlimited** opens (the daily quota does not apply). The
subscription system does not exist yet, so this check SHALL currently return false;
the seam MUST be in place so future billing enables it by implementing one function
without changing the quota logic. When the quota is reached / an unlock is offered,
the access state SHALL carry an **upsell** signal (a placeholder now) so that, once
subscriptions exist, the client can nudge the user to subscribe at those moments
without a contract change; the app's placeholder SHALL reuse the reward shop's
existing "temporary premium access — coming later" wording.
