...
dedup/grouping), `title_norm` (normalised for text search), `is_piano` /
instrumentation, `key_fifths`, `time_sig`, `measure_count`, and, when available,
`language` and `voicing`. Source-specific extras SHALL be stored in a `metadata`
JSON column rather than requiring a per-source migration.
