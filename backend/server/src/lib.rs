...
// The music HTTP surface now lives in the music module itself (change:
// harden-module-boundaries, group 5): a module owns its whole external surface, gRPC
// and HTTP alike, and the composition root only mounts it. Re-exported so the routes'
// names stay where callers already look for them.
pub use cymbra_music::score_preview_http::{ScorePreviewState, score_preview_router};
pub use cymbra_music::soundfont_http::{JwtAuth, SoundfontAuth, SoundfontState, soundfont_router};
...
mod build_tests {
    ...
    fn both_services_mount() {
        ...
        let cfg = AuthConfig::new(
            ...
            vec!["music".into()],
            12,
            cymbra_auth::AuthLimits::with_default_ceilings(
                3,
