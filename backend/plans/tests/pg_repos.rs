...
use cymbra_plans::pg::{
    ...
    PgMembershipRepo, PgStoreTesterRepo,
};
use cymbra_plans::{
    ...
    MembershipSource, NewCampaign, Source, StoreTesterRepo, codes,
};
...
/// The store-tester mark (change: scope-sandbox-to-tester-accounts). Exercised
...
#[tokio::test]
#[ignore = "needs docker compose (Postgres) up with per-module roles"]
async fn store_tester_marks_are_set_read_in_batch_and_cleared() {
    let pool = pool().await;
    let repo = PgStoreTesterRepo::new(pool.clone());
    let a = Uuid::now_v7();
    let b = Uuid::now_v7();
    let c = Uuid::now_v7();

    assert!(!repo.is_tester(&a.to_string()).await.unwrap());
    ...
    let among = repo
        .testers_among(&[a.to_string(), b.to_string(), c.to_string()])
        .await
        .unwrap();
    ...
    // An id that is not a uuid is simply not a tester — never an error that a
    // webhook would retry.
    assert!(!repo.is_tester("$RCAnonymousID:abc").await.unwrap());
    assert!(
        repo.testers_among(&["$RCAnonymousID:abc".to_string()])
            .await
            .unwrap()
            .is_empty()
    );
    ...
    assert!(!repo.is_tester(&b.to_string()).await.unwrap());
}
