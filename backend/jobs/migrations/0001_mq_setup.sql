-- Vendored from sqlxmq 0.6.0 (Diggsey/sqlxmq, 20210316025847_setup.up.sql),
-- kept verbatim so the engine can be re-vendored on upgrade. Two changes only:
--   * `CREATE EXTENSION "uuid-ossp"` is hoisted to the dev bootstrap
--     (db/init/01-schemas-roles.sql) because the `cymbra-jobs` migrator runs as
--     the non-superuser `worker_svc` role.
--   * objects are unqualified, so they land in `worker_svc`'s pinned
--     search_path (`jobs`) — see design D3 / the spike findings.
