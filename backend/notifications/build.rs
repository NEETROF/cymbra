...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Generates the `cymbra.notifications.v1` protobuf types + the tonic SERVER
    // stub (NotificationService — token registry, consent, timezone).
    tonic_build::configure()
        .build_client(false)
        .compile_protos(&["proto/notifications.proto"], &["proto"])?;
    Ok(())
}
