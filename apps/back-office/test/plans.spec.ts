...
describe("plans store", () => {
  ...
  // The sandbox-account mark decides whether a SANDBOX purchase counts. It grants
  // nothing on its own, but it is audited, so it goes through the same
  // mutate + relookup path as every other plan change.
  it("marks an account as accepting sandbox purchases, then re-runs the lookup so the box reflects the server", async () => {
    const { clients, state } = makeFakeClients({ lookup });
    setClientsForTest(clients);
    const store = usePlansStore();
    await store.lookup("ada");

    const outcome = await store.setSandboxAccount({ target: { userId: UUID }, enabled: true, reason: "app review" });

    expect(outcome.status).toBe("success");
    expect(state.setSandboxAccountCalls).toEqual([{ userId: UUID, handle: "", enabled: true, reason: "app review" }]);
    expect(state.lookupCalls).toHaveLength(2);
  });
  ...
  it("clearing the mark is the same call with enabled false", async () => {
    ...
    await store.setSandboxAccount({ target: { handle: "ada" }, enabled: false, reason: "review over" });

    expect(state.setSandboxAccountCalls).toEqual([
      { userId: "", handle: "ada", enabled: false, reason: "review over" },
    ]);
  });
  ...
  it("a refused mark lands in `op` as an error", async () => {
    ...
    (clients.plans as unknown as { setSandboxAccount: () => Promise<never> }).setSandboxAccount = () =>
      Promise.reject(new ConnectError("nope", Code.PermissionDenied));
    setClientsForTest(clients);
    const store = usePlansStore();
    await store.lookup("ada");

    const outcome = await store.setSandboxAccount({ target: { userId: UUID }, enabled: true, reason: "r" });
  ...
  it("passes the plan × beta criterion through and returns the resolved ids", async () => {
    ...
    expect(state.idsByPlanCalls).toEqual([
      { plan: "trial", betaCampaignKey: "midi-drums", sandboxAccountsOnly: false },
    ]);
