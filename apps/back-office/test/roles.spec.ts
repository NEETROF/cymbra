...
describe("roles store", () => {
  ...
  // The sandbox-account filter is a plans attribute on an identity-owned directory,
  // so it takes the same pre-resolve route as the plan and beta filters.
  it("the sandbox-account filter alone still pre-resolves ids", async () => {
    const accounts = [{ userId: "u1", handle: "ada", rolesByScope: [] }];
  ...
  it("the sandbox-account filter composes with a plan filter", async () => {
    const { clients, state } = makeFakeClients({ accounts: [], idsByPlan: ["u1"] });
    setClientsForTest(clients);
    const store = useRolesStore();
  ...
  it("a beta filter with an empty resolved set is an empty page without calling listAccounts", async () => {
    ...
    expect(state.idsByPlanCalls).toEqual([{ plan: "any", betaCampaignKey: "midi-drums", sandboxAccountsOnly: false }]);
