# `#welcome` — texte à coller / text to paste
...
Read the rules below before posting. Say hi in **#general** — we read everything.
```
...
Lis les règles ci-dessous avant de poster. Viens dire bonjour dans **#general** — on lit tout.
```
...
- **Rule 2 is deliberately two-tiered**: Discord's floor is 13, while being *named publicly*
  reuses Cymbra's own `min_public_sharing_age` (default 16) — the same gate the app applies to
  public profiles. Do not "simplify" it to one number; they protect different things.
- **Rule 3 mirrors the per-upload authorship attestation** in the app and the Terms clause that
  makes the uploader solely liable. Keep the wording aligned if the Terms change.
